...
//	}
	...
	g14[0] = 16;
	g14[1] = blknum * 16; // instead of N, lauch enough blocks 
	...
	err = clSetKernelArg(kernel[14], 2, sizeof(int), &blk_rows);
	if(err != 0) { printf("%d\n",err); OCL_CHECK(err); exit(1);}

 	err = clSetKernelArg(kernel[14], 3, sizeof(int), &D);
	if(err != 0) { printf("%d\n",err); OCL_CHECK(err); exit(1);}
	...
	for(stateNum = 0; stateNum < N ; ++stateNum)
	{
		...
		err = clSetKernelArg(kernel[14], 4, sizeof(uint), &offset);
		if(err != 0) { printf("%d\n",err); OCL_CHECK(err); exit(1);}

		err = clEnqueueNDRangeKernel(queue, kernel[14], 2, NULL, g14, l14, 0, NULL, NULL);
		OCL_CHECK(err);
	}
