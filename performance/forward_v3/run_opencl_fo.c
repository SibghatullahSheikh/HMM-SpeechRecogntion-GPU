...
void run_opencl_fo(HMM *word)
{
	...
	cl_command_queue queues[2];           // command queue
...
/*
	...
	*/
	...
	// partition  the device into 2 sub devices 
    cl_device_id device_ids[2];
    const cl_device_partition_property props[] = {CL_DEVICE_PARTITION_EQUALLY, num_cu/2, 0};

    err = clCreateSubDevices(device_id, props, 2, device_ids, NULL);
	OCL_CHECK(err);


	// Create a context  
	context = clCreateContext(0, 2, device_ids, NULL, NULL, &err);
	OCL_CHECK(err);

	// Create command queues for all sub devices 
	for (i=0; i<2; ++i)
	{
		queues[i] = clCreateCommandQueue(context, device_ids[i], CL_QUEUE_PROFILING_ENABLE, &err);
		OCL_CHECK(err);
	}
	...
	// build programs for all sub-devices
	err = clBuildProgram(program, 2, &device_ids, options, NULL, NULL);
	if(err != CL_SUCCESS){
		printCompilerOutput(program, device_ids[0]);
		printCompilerOutput(program, device_ids[1]);
	}
	OCL_CHECK(err);
